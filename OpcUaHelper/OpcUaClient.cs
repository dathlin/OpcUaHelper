...
namespace OpcUaHelper
{
    ...
    public class OpcUaClient
    {
        ...
        public (bool res, string msg) WriteNodesWithResult(string[] tags, object[] values)
        {
            WriteValueCollection valuesToWrite = new WriteValueCollection();
            ...
            foreach (var r in results)
            {
                ...
                {
                    result = false;
                    msg = $"节点 '{tags[j]}' 写入失败,{new ServiceResultException(results[j]).Message}";
                    break;
                }
                j++;
            }
        ...
        public OpcNodeAttribute[] ReadNoteAttributes(string tag)
        {
            ...
            for (int ii = 0; ii < results.Count; ii++)
            {
                ...
                if (ii < startOfProperties)
                {
                    ...
                    item.Name = Opc.Ua.Attributes.GetBrowseName(nodesToRead[ii].AttributeId);

                    // display any unexpected error.
                    if (StatusCode.IsBad(results[ii].StatusCode))
                    {
                        item.Type = Utils.Format("{0}", Opc.Ua.Attributes.GetDataTypeId(nodesToRead[ii].AttributeId));
        ...
        public DataValue[] ReadNoteDataValueAttributes(string tag)
        {
            ...
            // attempt to read all possible attributes.
            // 尝试着去读取所有可能的特性
            for (uint ii = Opc.Ua.Attributes.NodeId; ii <= Opc.Ua.Attributes.UserExecutable; ii++)
            {
                ReadValueId nodeToRead = new ReadValueId();
                nodeToRead.NodeId = sourceId;
                nodeToRead.AttributeId = ii;
